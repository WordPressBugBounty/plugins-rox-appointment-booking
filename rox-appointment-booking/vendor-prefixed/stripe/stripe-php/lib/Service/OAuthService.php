...
namespace RoxAppointmentBookingVendors\Stripe\Service;
...
class OAuthService extends AbstractService
{
    ...
    public function authorizeUrl($params = null, $opts = null)
    {
        ...
        $query = \RoxAppointmentBookingVendors\Stripe\Util\Util::encodeParameters($params);
    ...
    private function _getClientId($params = null)
    {
        ...
        if (null === $clientId) {
            ...
            throw new \RoxAppointmentBookingVendors\Stripe\Exception\AuthenticationException($msg);
        }
        return $clientId;
    }
    ...
    private function _parseOpts($opts)
    {
        if (\is_array($opts)) {
            if (\array_key_exists('connect_base', $opts)) {
                ...
                throw new \RoxAppointmentBookingVendors\Stripe\Exception\InvalidArgumentException('Use `api_base`, not `connect_base`');
            }
        }
        return \RoxAppointmentBookingVendors\Stripe\Util\RequestOptions::parse($opts);
    }
    /**

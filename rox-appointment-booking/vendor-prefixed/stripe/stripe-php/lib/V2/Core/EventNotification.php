...
namespace RoxAppointmentBookingVendors\Stripe\V2\Core;

use RoxAppointmentBookingVendors\Stripe\Events\UnknownEventNotification;
use RoxAppointmentBookingVendors\Stripe\Reason;
use RoxAppointmentBookingVendors\Stripe\RelatedObject;
use RoxAppointmentBookingVendors\Stripe\Util\EventNotificationTypes;
/**
...
abstract class EventNotification
{
    ...
    public function __construct($json, $client)
    {
        ...
        if (\array_key_exists('context', $json) && null !== $json['context']) {
            $this->context = \RoxAppointmentBookingVendors\Stripe\StripeContext::parse($json['context']);
        }
    ...
    public static function fromJson($jsonStr, $client)
    {
        ...
        if (isset($json['object']) && 'event' === $json['object']) {
            throw new \RoxAppointmentBookingVendors\Stripe\Exception\UnexpectedValueException('You passed a webhook payload to StripeClient::parseEventNotification, which expects an event notification. Use Webhook::constructEvent instead.');
        }
    ...
    {
        $response = $this->client->rawRequest('get', "/v2/core/events/{$this->id}", null, ['stripe_context' => $this->context, 'headers' => ['Stripe-Request-Trigger' => 'event=' . $this->id]], null, ['fetch_event']);
    ...
    protected function fetchRelatedObject()
    {
        ...
        $options = ['headers' => ['Stripe-Request-Trigger' => 'event=' . $this->id]];
        ...
        return $this->client->deserialize($response->body, \RoxAppointmentBookingVendors\Stripe\Util\Util::getApiMode($this->related_object->url));
    }
}

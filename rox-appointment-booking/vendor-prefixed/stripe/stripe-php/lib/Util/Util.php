...
use RoxAppointmentBookingVendors\Stripe\StripeObject;
use RoxAppointmentBookingVendors\Stripe\V2\DeletedObject;
abstract class Util
{
            ...
            if ($isV2DeletedObject) {
                ...
                if ('v2' === $apiMode && 'v2.core.event' === $resp['object']) {
                    ...
                    if (\array_key_exists('type', $resp) && \array_key_exists($resp['type'], $eventTypes)) {
                        ...
                        $class = \RoxAppointmentBookingVendors\Stripe\V2\Core\Event::class;
                    }
                }
                ...
                $class = \RoxAppointmentBookingVendors\Stripe\V2\Collection::class;
            } else {
    ...
    {
        if ($h instanceof \RoxAppointmentBookingVendors\Stripe\ApiResource) {
            return $h->id;
        }
        if (static::isList($h)) {
            $results = [];

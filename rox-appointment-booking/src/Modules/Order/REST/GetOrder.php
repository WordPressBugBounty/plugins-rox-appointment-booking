...
class GetOrder extends AbstractREST
{
    ...
    protected function getSingleOrderData(OrderModel $order): array
    {
        ...
        return [
            ...
            'payment_status' => $order->payment_status,
            'payment_status_label' => rox_appointment_booking_get_payment_status_label($order->payment_status),
            'deposit_amount' => (float) ($order->deposit_amount ?? 0),
            'deposit_amount_formatted' => $currencySymbol . number_format((float) ($order->deposit_amount ?? 0), 2, '.', ''),
            'amount_due_later' => (float) ($order->amount_due_later ?? 0),
            'amount_due_later_formatted' => $currencySymbol . number_format((float) ($order->amount_due_later ?? 0), 2, '.', ''),
